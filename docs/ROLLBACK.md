...
**刷新は完了し、main にマージ・公開済み。**

| 項目 | 内容 |
|---|---|
| マージコミット | `d59cb38ac42fa5d5eb627408de01b9d6deb3c3b5` |
| 刷新前の状態 | タグ `v1-before-redesign`（コミット `92a39b0`） |
| 内訳 | 14コミット。`git log d59cb38^2` で辿れる |
| 作業ブランチ | `feature/lp-redesign`（削除していない） |

### Instant Rollback がいま何手で効くか

**この節は本番へ push するたびに古くなる。push したら必ず読み直すこと。**

Vercel の本番デプロイは、main へ push するたびに1つ増える。
Hobby プランで戻せるのは**ひとつ前の本番デプロイだけ**なので、
「1手で刷新前に戻れる」のは**マージ後の最初のデプロイでいる間だけ**である。

```
  デプロイ1（刷新前）  ← 戻りたい先
  デプロイ2（刷新後）  ← いまここ。ひとつ前 = デプロイ1 なので1手で戻れる

  デプロイ3 を作ると…
  デプロイ2（刷新後）  ← ひとつ前がこれになる
  デプロイ3（刷新後）  ← いまここ。1手で戻っても刷新後のまま
```

**main に push を重ねると、Vercel からは刷新前に戻せなくなる。**
「画面が変わらない変更だから安全」とは言えない。ドキュメントの修正でも、
README の誤字直しでも、push すれば本番デプロイが1つ増え、
戻り先が1つ手前へずれる。ずれるのは*戻れる地点*であって、
*画面の見た目*ではない。

問題が見つかるまで main への push を止めておくと、この1手が残る。

なお git 側（下記2）にはこの制限がない。**タグからはいつでも戻せる。**
push を重ねてしまった場合は git の手順を使うこと。
